...
void eventMixer()
{
	...
	std::ifstream infile("fourmuoneventmix_UpsilonMuonandTripleMu5.txt");
	...
	//for(int i=0; i<1000; i++)
	{
		...
		for(int j=0; j<P4Vector2.size(); j++)
		{
				...
				)
			{
				...
				// cout<<"px="<<tempV1p4.Px()<<" original p4 px="<<P4Vector1[i].Px()<<endl;
...
/*
	...
	myEventMixingMassHistfinebin->GetXaxis()->SetTitle("m((#mu^{+}#mu^{-})_{1}(#mu^{+}#mu^{-})_{2} [GeV]");
	myEventMixingMassHistfinebin->GetYaxis()->SetTitle("Candidates/100 MeV");
	myEventMixingMassHistfinebin->Fit("pol8","","",12.0,35.0); // 12-35	
*/	
	...
	myEventMixingMassHistfinebin->GetXaxis()->SetTitle("m((#mu^{+}#mu^{-})_{1}(#mu^{+}#mu^{-})_{2} [GeV]");
	myEventMixingMassHistfinebin->GetYaxis()->SetTitle("Candidates/100 MeV");
	myEventMixingMassHistfinebin->Fit("pol7","","",12.0,35.0);


	// myEventMixingMassHist1->Fit("pol8","","",11.8,35);
	// myFourMuonMassHist->Fit("pol8","","",11.8,35);
	// TF1 *h = new TF1("h","32791.9051296436155098490417004-11873.8840660185251181246712804*x+1822.55372655460359965218231082*pow(x,2)-155.263906936737214437016518787*pow(x,3)+8.05550999979142190454695082735*pow(x,4)-0.261428422539682792091753071873*pow(x,5)+0.0051958940517187737415105708294*pow(x,6)-5.79455963296762602884187876029e-05*pow(x,7)+2.78107718132960800570444665977e-07*pow(x,8)",11.8,35.00);
	...
	*/
	...
	myFourMuonReversedV1V2PxPyHist->Fit("pol8","","",14.0,35.0);
