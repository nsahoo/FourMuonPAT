...
void UpperCrossingsToy() 
{

	TFile* myhbk = new TFile ("myhbk.root","recreate");
	TH1F* myqmHist = new TH1F("myqmHist","myqmHist",600,14.0,26.0);
	TH1F* mypValHist = new TH1F("mypValHist","mypValHist",600,14.0,26.0);
	TH2F* my2DHist = new TH2F("my2DHist","my2DHist",10.0,18.0,19.0,100.0,-11360.0,-11280.0);

	TFile *f1 = TFile::Open("CombinedVertexCut10em30.root");
	...
	RooRealVar   MyMean1  ("MyMean1"    ,"MyMean1"  /*,18.4532*/,14.0,26.0 );
	...
	RooRealVar c1("c1", "c1", -3.91832e-02,  -15., 15.0);
	RooRealVar c2("c2", "c2", -6.01975e-01,  -15., 15.0);
	RooRealVar c3("c3", "c3",  3.24216e-01,  -15., 15.0);
	RooRealVar c4("c4", "c4", -6.46913e-02,  -15., 15.0);
	...
	c1.setVal(-3.57157e-01);
	c2.setVal(-2.73907e-01);
	c3.setVal( 2.34310e-01);
	c4.setVal(-4.94928e-02);
	c1.setConstant();
	c2.setConstant();
	c3.setConstant();
	c4.setConstant();
	...
	// RooAbsPdf *  BkgPolPdf = new RooGenericPdf("BkgPolPdf","BkgPolPdf","583.812817138705327124625910074-57.6788294160430297097263974138*varMyXmass+2.16305828930716970148750988301*pow(varMyXmass,2)-0.0359286182737813264420623227124*pow(varMyXmass,3)+0.000221670948838491213127993573195*pow(varMyXmass,4)",RooArgSet(varMyXmass));
	...
	Double_t tmpseed = 0.0;
	RooRandom::randomGenerator()->SetSeed(seed);
	gRandom->SetSeed(seed);

	RooDataHist* alldata;
	int mybcknumtmp=gRandom->Poisson(2098);
	alldata = BkgPolPdf->generateBinned(varMyXmass,mybcknumtmp);
...
#define NscanWhole 600
#define scanstep 0.02
	double myScanNLLAr[NscanWhole];

	for (int i = 1; i <= NscanWhole; ++i)
	{
		double mymean=14.0 +(double)i*scanstep;
		MyMean1.setVal( mymean );
		MyMean1.setConstant();
		myfitresult= mytotalPdf.fitTo(*alldata,Minos(kTRUE), Save(1));
		double myscanNLL = myfitresult.minNll();
		myScanNLLAr[i] = myscanNLL;

		myqmHist->SetBinContent(i, 2*(NullFitNLL-myscanNLL)  );
		mypValHist->SetBinContent(i,TMath::Prob(2*(NullFitNLL-myscanNLL),1)/2);
	}

	for (int i = 0; i < NscanWhole; ++i)
	{
		
		cout<<"myScanNLL = "<<myScanNLLAr[i]<<" myNullNLL = "<<NullFitNLL<<endl;
...
#define scanstp 0.005
	...
	canvas->cd();
	myqmHist->SetTitle("");
	myqmHist->GetXaxis()->SetTitle("m(#mu^{+}#mu^{-}#mu^{+}#mu^{-}) [GeV]");
	myqmHist->GetYaxis()->SetTitle("q(m)");
	myqmHist->GetXaxis()->SetTitleSize(0.06);
	myqmHist->GetYaxis()->SetTitleSize(0.06);
	myqmHist->GetYaxis()->SetTitleOffset(0.7);
	myqmHist->GetXaxis()->SetTitleOffset(0.7);
	myqmHist->Draw();
	TLine *lineUpCross = new TLine(14.0,0.5,26.0,0.5);
	lineUpCross->SetLineColor(kRed);
	lineUpCross->Draw();
	...
	canvas1->cd();
	mypValHist->SetTitle("");
	mypValHist->GetXaxis()->SetTitle("m(#mu^{+}#mu^{-}#mu^{+}#mu^{-}) [GeV]");
	mypValHist->GetYaxis()->SetTitle("Local p-value");
	mypValHist->GetXaxis()->SetTitleSize(0.06);
	mypValHist->GetYaxis()->SetTitleSize(0.06);
	mypValHist->GetYaxis()->SetTitleOffset(0.7);
	mypValHist->GetXaxis()->SetTitleOffset(0.7);
	mypValHist->Draw();
	// TLine *lineUpCross1 = new TLine(14.0,0.5,26.0,0.5);
	// lineUpCross1->SetLineColor(kRed);
	// lineUpCross->Draw();
