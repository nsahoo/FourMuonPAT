...
void myntuple::Loop()
{  
	ofstream myoutfilesignal4Cristina("myfourmuonsignal4Cristina_RUNNO.txt");

	ofstream myoutfilesignal7cats("myfourmuonsignalGigi7cats_RUNNO.txt");


	ofstream myoutfilesignal("myfourmuonsignal_RUNNO.txt");
	ofstream myoutfileEvtNum("myfourmuonEvtNum_RUNNO.txt");
	ofstream myoutfileevtmix("myfourmuonevtmix_RUNNO.txt");
	ofstream myoutfiledimu("mydimuon_RUNNO.txt");

	ofstream myoutfileJYforKamuran("myJYforKamuran_RUNNO.txt");
	ofstream myoutfileJYforMaksat("myJYforMaksat_RUNNO.txt");



	ofstream myoutfilesignal1("myfourmuonsignal1_RUNNO.txt");
	ofstream myoutfilesignal2("myfourmuonsignal2_RUNNO.txt");


	TFile* myhbk = new TFile ("myhbk_MuOnia_RUNNO.root","recreate");

	//for ARC comments
	ofstream myARCoutNmuTCHG("myARCoutNmuTCHG_RUNNO.txt");
	myARCoutNmuTCHG<<"runNum "<<"lumiNum "<<"evtNum "<<"myGoodSoftMu "<<"myTotalChg "<<endl;

	ofstream myARCoutNTightMu("myARCoutNTightMu_RUNNO.txt");
 ...
 //end ARC
	...
	TH1F* myMuMuonlyPull = new TH1F("myMuMuonlyPull","myMuMuonlyPull",100000,0,500.0);  
	myMuMuonlyPull->GetXaxis()->SetTitle("m(#mu^{+}#mu^{-}) GeV");
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) {
		...
		//ARC suggestion
			...
			for(int myi=0; myi<nmumuonly; myi++) {
				...
				int myNumGoodLooseMuonIlse = (*muIsGoodSoftMuonNewIlse)[ (*mumuonlymu1Idx)[myi]] + (*muIsGoodSoftMuonNewIlse)[ (*mumuonlymu2Idx)[myi]];
				...
				if(1
					...
					myMuMuonlyPull->Fill( (*mumuonlyMass)[myi]-9.46 );
...
// 	//cout<<endl;
	...
	//----------------------------------------------------------------------------------------------------------------------------
//  	for(int i=0; i<3; i++) { 	  
// 	  cout<<"same i="<<i<<", myMumu1Mumu2DisZ="<<myMumu1Mumu2DisZ[i]<<",1234="<<(*My1234JpsiDisZ)[myFourMuIdx]<<",1324="<<(*My1324JpsiDisZ)[myFourMuIdx]<<",1423="<<(*My1423JpsiDisZ)[myFourMuIdx]<<", vtx  prob="<<myMupMumVtxProb[i][0]<<",myMupMumMass 0="<<myMupMumMass[i][0]<<",myMupMumMass 1="<<myMupMumMass[i][1]<<endl;
//  	}
//  	//cout<<endl;
	...
	//----------------------------------------------------------------------------------------------------------------------------
	/*	for(int i=0; i<3; i++) { 	  
//	cout<<"same i="<<i<<", myMumu1Mumu2DisZ="<<myMumu1Mumu2DisZ[i]<<",1234="<<(*My1234JpsiDisZ)[myFourMuIdx]<<",1324="<<(*My1324JpsiDisZ)[myFourMuIdx]<<",1423="<<(*My1423JpsiDisZ)[myFourMuIdx]<<", vtx  prob="<<myMupMumVtxProb[i][0]<<",myMupMumMassErr 0="<<myMupMumMassErr[i][0]<<",myMupMumMassErr 1="<<myMupMumMassErr[i][1]<<endl;
cout<<"same i="<<i<<",(vtx prob, mass, masserr)_0=("<<myMupMumVtxProb[i][0]<<","<<myMupMumMass[i][0]<<","<<myMupMumMassErr[i][0]<<"),(vtx prob, mass, masserr)_1=("<<myMupMumVtxProb[i][1]<<","<<myMupMumMass[i][1]<<","<<myMupMumMassErr[i][1]<<")"<<endl;
		}
//  	//cout<<endl;
*/
...
//end added by yik to check EBE fro ARC
	...
	*/
	...
	/*
	//test new soft muonID || old soft muonID w/ TMOneStationLoose
	isLooseMuon[0]=(*muIsGoodSoftMuonNewIlse)[ (*MyFourMuonMu1Idx)[myFourMuIdx]] ||  (*muIsGoodLooseMuon)[ (*MyFourMuonMu1Idx)[myFourMuIdx]];   
	isLooseMuon[1]=(*muIsGoodSoftMuonNewIlse)[ (*MyFourMuonMu2Idx)[myFourMuIdx]] ||  (*muIsGoodLooseMuon)[ (*MyFourMuonMu2Idx)[myFourMuIdx]];
	isLooseMuon[2]=(*muIsGoodSoftMuonNewIlse)[ (*MyFourMuonMu3Idx)[myFourMuIdx]] ||  (*muIsGoodLooseMuon)[ (*MyFourMuonMu3Idx)[myFourMuIdx]];   
	isLooseMuon[3]=(*muIsGoodSoftMuonNewIlse)[ (*MyFourMuonMu4Idx)[myFourMuIdx]] ||  (*muIsGoodLooseMuon)[ (*MyFourMuonMu4Idx)[myFourMuIdx]];
	*/
	...
	bool myVVmasscut;
			if( mychannel==11 ) {  //Y(1S) mumu
				...
				*/
				...
				//used EBE error
					myVVmasscut= fabs((fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M() -mymV )<= mynsigma*1.105*myMupMumMassErr[mypidx][0]  
										//myVVmasscut= (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M() < (mymV - mynsigma*myY1sResp1) 
											//           && (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M() > (mymV - 2*mynsigma*myY1sResp1)
																	//to check background event to see if we can have a bck shape.
					&& (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M() <= ( mymV-mynsigma*1.105*myMupMumMassErr[mypidx][1] )
					//&& (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M() <= 9.0  //4; 5, 7; 
					&& myNumGoodTightMuon>=2; //if one of the V is Upsilon
...
//    myoutfileEvtNum<<runNum<<" "<<evtNum<<" "<<lumiNum<<endl;
						...
						<<" "<<(*MyFourMuonCTau)[ myFourMuIdx ]<<" "<<runNum<<" "<<evtNum<<" "<<lumiNum<<endl;
...
if( fabs((fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M()-3.096919)> myMupMumMassErr[myanotherpidx][0]*3.0 
	&& fabs((fitMup4vect[ muanotherIdxp21 ]+ fitMup4vect[ muanotherIdxp22 ]).M()-3.096919)>  myMupMumMassErr[myanotherpidx][1]*3.0
	&& fabs( (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M()-3.096919)> myMupMumMassErr[mypidx][1]*3.0
	&& fabs((fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M()-1.01946)> myMupMumMassErr[myanotherpidx][0]*3.0
	&& fabs((fitMup4vect[ muanotherIdxp21 ]+ fitMup4vect[ muanotherIdxp22 ]).M()-1.01946)> myMupMumMassErr[myanotherpidx][1]*3.0
	&& fabs( (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M()-1.01946)> myMupMumMassErr[mypidx][1]*3.0
	&& fabs((fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M()-0.786)>myMupMumMassErr[myanotherpidx][0]*3.0
	&& fabs((fitMup4vect[ muanotherIdxp21 ]+ fitMup4vect[ muanotherIdxp22 ]).M()-0.786)>myMupMumMassErr[myanotherpidx][1]*3.0
	&& fabs( (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M()-0.786)> myMupMumMassErr[mypidx][1]*3.0
	) {
	myMuMuMuMumassV1mumubelowV1nolightmeson->Fill( myfourmuonmass );
	myoutfilesignal1<<myfourmuonmass<<" "<<(*MyFourMuonMassErr)[myFourMuIdx]<<endl;
}
else {
