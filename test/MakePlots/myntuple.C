...
// TString myevtmixoutname=outputname+"myevtmix.txt";
// ofstream myoutfileevtmix(myevtmixoutname);
...
//  ofstream myoutfileJYforMaksat("myJYforMaksat.txt");
	...
	// fill the wrong dimu combinations in signal and sideband
	TString mywrongdimu1Signaloutname=outputname+"mywrongdimu1Signal.txt";
	ofstream mywrongdimu1Signal(mywrongdimu1Signaloutname);

	TString mywrongdimu2Signaloutname=outputname+"mywrongdimu2Signal.txt";
	ofstream mywrongdimu2Signal(mywrongdimu2Signaloutname);

	TString mywrongdimu1Sidebandoutname=outputname+"mywrongdimu1Sideband.txt";
	ofstream mywrongdimu1Sideband(mywrongdimu1Sidebandoutname);

	TString mywrongdimu2Sidebandoutname=outputname+"mywrongdimu2Sideband.txt";
	ofstream mywrongdimu2Sideband(mywrongdimu2Sidebandoutname);



	//ofstream myoutfilesignal1("myfourmuonsignal1.txt");
	//ofstream myoutfilesignal2("myfourmuonsignal2.txt");

	TString mylowsidebandname=outputname+"mylowsideband1.txt";
	ofstream mylowsideband1(mylowsidebandname);
...
//  myARCoutNTightMu<<"runNum "<<"lumiNum "<<"evtNum "<<"myGoodTightMu "<<endl;
 ...
 //end ARC
	...
	TH1F* myMuMuMuMumassV1mumubelowV1nolightmesonDiff = new TH1F("myMuMuMuMumassV1mumubelowV1nolightmesonDiff","myMuMuMuMumassV1mumubelowV1nolightmesonDiff",10000,0,1000.0);
 ...
 //to save emeory, comment out 2D histograms
 /*
...
myMuMu1ptVsMuMu2ptat18dot5GeV->GetYaxis()->SetTitle("p_{T}(#mu^{+}#mu^{-})_{low} GeV");
 ...
 TH2F* myDalitzmY1smupmasssqVsmmumumasssqatSideband = new TH2F("myDalitzmY1smupmasssqVsmmumumasssqatSideband","myDalitzmY1smupmasssqVsmmumumasssqatSideband",4000,0,400.0, 10000, 0, 100.0);  
	...
	TH2F* myDeltaEtavsDeltaPhi4minOppositeMuat18dot5GeV = new TH2F("myDeltaEtavsDeltaPhi4minOppositeMuat18dot5GeV","myDeltaEtavsDeltaPhi4minOppositeMuat18dot5GeV",500,0,5,500,0,5);
	TH2F* myDeltaEtavsDeltaPhi4minSameSignMuat18dot5GeV = new TH2F("myDeltaEtavsDeltaPhi4minSameSignMuat18dot5GeV","myDeltaEtavsDeltaPhi4minSameSignMuat18dot5GeV",500,0,5,500,0,5);
 ...
 */
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) 
	{
		...
		int myTotalChg=0;
		int mySoftMuIdx[100];  
		//assume maximum good muon is 100
	...
	//	double mymV[2]={9.46,3.0967};
			double mymV;   
	//3.096919 for jpsi, 3.686 for psi2s, 9.46 for Y(1s),10.02326,10.3552
	//code with simplest and safest way to make sure 
			...
			if(myordermode==1)
			//sort to make m11>m12, m21>m22, m21>m11
			{
				for(int i=1; i<3; i++) 
				{
		...
		else if(myordermode==2)
		{
			for(int i=1; i<3; i++) 
			{
				if( (fitMup4vect[ myCombIdx[i].p11 ]+ fitMup4vect[ myCombIdx[i].p12 ]).M()<(fitMup4vect[ myCombIdx[i].p21 ]+ fitMup4vect[ myCombIdx[i].p22 ]).M()  ) swap_one_paired_pair( myCombIdx[i] );
			}	
		...
		else if(myordermode==3)
		{
			for(int i=1; i<3; i++) 
			{
				if( fabs((fitMup4vect[ myCombIdx[i].p11 ]+ fitMup4vect[ myCombIdx[i].p12 ]).M()-mymV) > fabs((fitMup4vect[ myCombIdx[i].p21 ]+ fitMup4vect[ myCombIdx[i].p22 ]).M()-mymV)  ) swap_one_paired_pair( myCombIdx[i] ); 
			}
				//sort to make sure that |myCombIdx[i][1]-pair1 mass - m(V)| < |myCombIdx[i][2]-pair1- m(V)| ,  V=Y(1S), J/psi
			if( fabs((fitMup4vect[ myCombIdx[1].p11 ]+ fitMup4vect[ myCombIdx[1].p12 ]).M()-mymV )> fabs((fitMup4vect[ myCombIdx[2].p11 ]+ fitMup4vect[ myCombIdx[2].p12 ]).M()-mymV ) )
				swap_two_paired_pair( myCombIdx[1],  myCombIdx[2] );
...
// 	cout<<" ctauerr, 01&23, 02&13, 03&12="<<(*MyJpsi1CTauErr_Mu12)[myFourMuIdx]<<","<<(*MyJpsi2CTauErr_Mu34)[myFourMuIdx]<<",  "<<(*MyJpsi3CTauErr_Mu13)[myFourMuIdx]<<","<<(*MyJpsi4CTauErr_Mu24)[myFourMuIdx]<<",  "<<(*MyJpsi5CTauErr_Mu14)[myFourMuIdx]<<","<<(*MyJpsi6CTauErr_Mu23)[myFourMuIdx]<<endl;
		...
		float myMupMumVtxProb[3][2];   
	//[2]-> V=Y1s, J/psi; 3-> 3 combinations; 2->pair 1 & pair 2
...
//           muIdxp11=myCombIdx[mypidx].p11; muIdxp12=myCombIdx[mypidx].p12; muIdxp21=myCombIdx[mypidx].p21; muIdxp22=myCombIdx[mypidx].p22;
				...
				if(myMupMumVtxProb[i][j]>0.005   
					 //impl the total chg==0 for this cut because it will be negative otherwise
...
//   	}
	...
	//int matchJpsiTrig=(int)(*muIsJpsiTrigMatch)[ (*MyFourMuonMu1Idx)[myFourMuIdx] ] + (int)(*muIsJpsiTrigMatch)[ (*MyFourMuonMu2Idx)[myFourMuIdx] ] + (int)(*muIsJpsiTrigMatch)[ (*MyFourMuonMu3Idx)[myFourMuIdx] ] + (int)(*muIsJpsiTrigMatch)[ (*MyFourMuonMu4Idx)[myFourMuIdx] ] ;
			...
			float myetacut=2.4;   
	//1.6, there is a bump at 31 GeV for Y(1S) mumu, default 2.4
			float minMuPt=2.0;  
	//2.0--default 1.5;2,5;3.0;3.5;  2.2
...
//test Slava's comment on overlap.
/*
&& (*MyFourMuonSharedSeg12)[myFourMuIdx] <=0 && (*MyFourMuonSharedSeg13)[myFourMuIdx] <=0 && (*MyFourMuonSharedSeg14)[myFourMuIdx] <=0 
&& (*MyFourMuonSharedSeg23)[myFourMuIdx] <=0 && (*MyFourMuonSharedSeg24)[myFourMuIdx] <=0 && (*MyFourMuonSharedSeg34)[myFourMuIdx] <=0
*/
...
//&& TrigThreeMuonJpsi
		...
		&& TrigThreeMuonUpsi
		 ...
		 //test deltaR effect           
		 //&& rawMup4vect[0].DeltaR( rawMup4vect[1] ) >0.6 && rawMup4vect[0].DeltaR( rawMup4vect[2] ) >0.6 && rawMup4vect[0].DeltaR( rawMup4vect[3] ) >0.6
		 //&& rawMup4vect[1].DeltaR( rawMup4vect[2] ) >0.6 && rawMup4vect[1].DeltaR( rawMup4vect[3] ) >0.6 && rawMup4vect[2].DeltaR( rawMup4vect[3] ) >0.6
...
{	
	...
	int NumDupARCEBE=0;
	int NumDup=0;   
			//we have two pairs, but only fill one time if at least one pair satisfy it
	for(int mypidx=1; mypidx<3; mypidx++) 
			//loop over the two combinations with zero chg
			//for(int mypidx=2; mypidx>0; mypidx--) //loop over the two combinations with zero chg, low mass combination first
	{
		...
		double myPhiResp1=0.006984+0.004505*myrap1+0.00426;  
		//0.00426 is its intrisince width
		double myOmegaResp1=0.006366+0.003277*myrap1+0.00849;  
		//+0.00849 is for its intrisic width
		...
		double myPhiResp2=0.006984+0.004505*myrap2+0.00426;   
		//0.00426 is its intrisince width
		double myOmegaResp2=0.006366+0.003277*myrap2+0.00849;  
		//+0.00849 is for its intrisic width
		...
		double myanotherPhiResp1=0.006984+0.004505*myanotherrap1+0.00426;  
		//0.00426 is its intrisince width
		double myanotherOmegaResp1=0.006366+0.003277*myanotherrap1+0.00849;  
		//+0.00849 is for its intrisic width
		...
		double myanotherPhiResp2=0.006984+0.004505*myanotherrap2+0.00426;  
		//0.00426 is its intrisince width
		double myanotherOmegaResp2=0.006366+0.003277*myanotherrap2+0.00849;  
		//+0.00849 is for its intrisic width
		...
		if(1
			...
			&& isLooseMuon[ muIdxp11 ] &&  isLooseMuon[ muIdxp12 ]
			&& isLooseMuon[ muIdxp21 ] &&  isLooseMuon[ muIdxp22 ]
...
//		      && (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M() <= ( mymV-mynsigma*myEBEscale*myMupMumMassErr[mypidx][1] )
					//&& (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M() >= 7.5  //4; 5, 7; 
					&& myNumGoodTightMuon>=2; //if one of the V is Upsilon		    
				}
			...
			//J/psi mumu, m(mumu)<m(Jpsi)
				//used EBE error
					...
					//&& fabs(rawMup4vect[0].Eta())<=1.6 && fabs(rawMup4vect[1].Eta())<=1.6
					//&& fabs(rawMup4vect[2].Eta())<=1.6 && fabs(rawMup4vect[3].Eta())<=1.6
					//&& (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).Pt()>=5.0
					//&& (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Pt()>=10.0
			...
			if(mychannel==11) {
			 ...
			 //&&(fitMup4vect[muIdxp21]+fitMup4vect[muIdxp22]).M()<=(mymV-mynsigma*myY1sResp2), assumed it is ordered 
				...
				//&& fabs( (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M()-0.78265)> myMupMumMassErr[mypidx][1]*myexclnsigma
					...
					mylowsideband1<<(mylowsidedimuonp4+myYstardimuonp4).M()<<" "<<(*MyFourMuonVtxCL)[myFourMuIdx]<<" "<<(rawMup4vect[0]+rawMup4vect[1]+rawMup4vect[2]+rawMup4vect[3]).Pt()<<" "<<runNum<<" "<<evtNum<<" "<<lumiNum<<endl;
	//		cout<<;
				}
...
//    myoutfileEvtNum<<runNum<<" "<<evtNum<<" "<<lumiNum<<endl;
			...
			//myoutfileevtmix<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).Px()<<" "<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).Py()<<" "<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).Pz()<<" "<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Px()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Py()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Pz()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M()<<" "<<fitMup4vect[ muIdxp11 ].Px()<<" "<<fitMup4vect[ muIdxp11 ].Py()<<" "<<fitMup4vect[ muIdxp11 ].Pz()<<" "<<fitMup4vect[ muIdxp12 ].Px()<<" "<<fitMup4vect[ muIdxp12 ].Py()<<" "<<fitMup4vect[ muIdxp12 ].Pz()<<" "<<fitMup4vect[ muIdxp21 ].Px()<<" "<<fitMup4vect[ muIdxp21 ].Py()<<" "<<fitMup4vect[ muIdxp21 ].Pz()<<" "<<fitMup4vect[ muIdxp22 ].Px()<<" "<<fitMup4vect[ muIdxp22 ].Py()<<" "<<fitMup4vect[ muIdxp22 ].Pz()<<" "<<endl;
						...
						myoutfilesignal<<myfourmuonmass<<" "<<(*MyFourMuonMassErr)[myFourMuIdx]<<" "<<(rawMup4vect[ muIdxp11 ]+rawMup4vect[ muIdxp12 ] + rawMup4vect[ muIdxp21 ]+rawMup4vect[ muIdxp22 ]).Pt()<<" "<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M()<<" "<<(rawMup4vect[ muIdxp11 ]+rawMup4vect[ muIdxp12 ]).Pt()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M()<<" "<<(rawMup4vect[ muIdxp21 ]+rawMup4vect[ muIdxp22 ]).Pt()<<" "<<rawMup4vect[ muIdxp11 ].Pt()<<" "<<rawMup4vect[ muIdxp12 ].Pt()<<" "<<rawMup4vect[ muIdxp21 ].Pt()<<" "<<rawMup4vect[ muIdxp22 ].Pt()<<" "<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ] + fitMup4vect[ muIdxp21 ] ).M()<<" "<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ] + fitMup4vect[ muIdxp22 ] ).M()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ] + fitMup4vect[ muIdxp11 ] ).M()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ] + fitMup4vect[ muIdxp12 ] ).M()<<" "<<(*MyFourMuonVtxCL)[myFourMuIdx]<<" "<<myMumu1Mumu2DisXY[mypidx]<<" "<<myMumu1Mumu2DisZ[mypidx]<<" "<<fabs( (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).Rapidity()-(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Rapidity() ) <<" "<<fabs( (fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).Rapidity()-(fitMup4vect[ muanotherIdxp21 ]+ fitMup4vect[ muanotherIdxp22]).Rapidity() ) <<" "<<(*MyFourMuonCTau)[ myFourMuIdx ]<<" "<<runNum<<" "<<evtNum<<" "<<lumiNum<<endl;

			//myoutfilesignal<<myfourmuonmass<<" "<<(*MyFourMuonMassErr)[myFourMuIdx]<<" "<<myMupMumMass[mypidx][0]<<" "<<myMupMumMassErr[mypidx][0]<<" "<< (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M()<<" "<<(fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M() << " "<< (fitMup4vect[ muanotherIdxp21 ]+ fitMup4vect[ muanotherIdxp22 ]).M()<<endl;
...
//    myfourmuonmass>=18.0 && myfourmuonmass<=19.5 
	 ...
	 //&& (fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M() >=2.9 && (fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M()<=3.3 

							)
						{
							myWrongdimu1signal->Fill( (fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M() );
							myWrongdimu2signal->Fill( (fitMup4vect[ muanotherIdxp12 ]+ fitMup4vect[ muanotherIdxp22 ]).M() );

							mywrongdimu1Signal<<myfourmuonmass<<" "<<(*MyFourMuonVtxCL)[myFourMuIdx]
							<<" "<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M()
							<<" "<<(fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M()
							<<" "<<fitMup4vect[ muanotherIdxp11 ].Px()<<" "<<fitMup4vect[ muanotherIdxp11 ].Py()<<" "<<fitMup4vect[ muanotherIdxp11 ].Pz()
							<<" "<<fitMup4vect[ muanotherIdxp12 ].Px()<<" "<<fitMup4vect[ muanotherIdxp12 ].Py()<<" "<<fitMup4vect[ muanotherIdxp12 ].Pz()
							<<" "<<runNum<<" "<<evtNum<<" "<<lumiNum<<endl;

							mywrongdimu2Signal<<myfourmuonmass<<" "<<(*MyFourMuonVtxCL)[myFourMuIdx]
							<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M()
							<<" "<<(fitMup4vect[ muanotherIdxp21 ]+ fitMup4vect[ muanotherIdxp22 ]).M()
							<<" "<<fitMup4vect[ muanotherIdxp21 ].Px()<<" "<<fitMup4vect[ muanotherIdxp21 ].Py()<<" "<<fitMup4vect[ muanotherIdxp21 ].Pz()
							<<" "<<fitMup4vect[ muanotherIdxp22 ].Px()<<" "<<fitMup4vect[ muanotherIdxp22 ].Py()<<" "<<fitMup4vect[ muanotherIdxp22 ].Pz()
							<<" "<<runNum<<" "<<evtNum<<" "<<lumiNum<<endl;
...
//(
//    (myfourmuonmass>=17.25 && myfourmuonmass<=18.0 )
//    ||
//    (myfourmuonmass>=19.5 && myfourmuonmass<=20.25 )
//)
...
//&& (fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M() >=2.9 && (fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M()<=3.3
							...
							)
						{
							...
							myWrongdimu2sideband->Fill( (fitMup4vect[ muanotherIdxp12 ]+ fitMup4vect[ muanotherIdxp22 ]).M() );

							mywrongdimu1Sideband<<myfourmuonmass<<" "<<(*MyFourMuonVtxCL)[myFourMuIdx]
							<<" "<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M()
							<<" "<<(fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M()
							<<" "<<fitMup4vect[ muanotherIdxp11 ].Px()<<" "<<fitMup4vect[ muanotherIdxp11 ].Py()<<" "<<fitMup4vect[ muanotherIdxp11 ].Pz()
							<<" "<<fitMup4vect[ muanotherIdxp12 ].Px()<<" "<<fitMup4vect[ muanotherIdxp12 ].Py()<<" "<<fitMup4vect[ muanotherIdxp12 ].Pz()
							...
							mywrongdimu2Sideband<<myfourmuonmass<<" "<<(*MyFourMuonVtxCL)[myFourMuIdx]
							<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M()
							<<" "<<(fitMup4vect[ muanotherIdxp21 ]+ fitMup4vect[ muanotherIdxp22 ]).M()
							<<" "<<fitMup4vect[ muanotherIdxp21 ].Px()<<" "<<fitMup4vect[ muanotherIdxp21 ].Py()<<" "<<fitMup4vect[ muanotherIdxp21 ].Pz()
							<<" "<<fitMup4vect[ muanotherIdxp22 ].Px()<<" "<<fitMup4vect[ muanotherIdxp22 ].Py()<<" "<<fitMup4vect[ muanotherIdxp22 ].Pz()
...
//test Joel's formular
/*
 TLorentzVector myupsilon1sp4=(fitMup4vect[ muIdxp11 ]+fitMup4vect[ muIdxp12 ]);
 //cout<<"upsilon m="<<myupsilon1sp4.M()<<", 4 mu mass="<<( myupsilon1sp4 + fitMup4vect[ muIdxp21 ]+fitMup4vect[ muIdxp22 ] ).M()<<endl;
 float myupEtot=sqrt(fitMup4vect[ muIdxp11 ].Vect().Mag2()+ 0.10566*0.10566)+sqrt(fitMup4vect[ muIdxp12 ].Vect().Mag2()+ 0.10566*0.10566); 
 cout<<"Old E="<<myupsilon1sp4.E()<<", modified E by Joel's formula="<<myupEtot<<endl;
*/
...
//myoutfilesignal1<<myfourmuonmass<<" "<<( myfourmuonmass-(fitMup4vect[ muIdxp11 ]+fitMup4vect[ muIdxp12 ]).M()+mymV)<<" "<<(*MyFourMuonMassErr)[myFourMuIdx]<<endl;
							myoutfilesignal1<<myfourmuonmass<<" "<<(*MyFourMuonMassErr)[myFourMuIdx]<<" "<<myMupMumMass[mypidx][0]<<" "<<myMupMumMassErr[mypidx][0]<<" "<< (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M()<<" "<<(fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M() << " "<< (fitMup4vect[ muanotherIdxp21 ]+ fitMup4vect[ muanotherIdxp22 ]).M()<<" "<<(*MyFourMuonVtxCL)[myFourMuIdx]<<" "<<(rawMup4vect[0]+rawMup4vect[1]+rawMup4vect[2]+rawMup4vect[3]).Pt()<<" "<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ] + fitMup4vect[ muIdxp21 ]  ).M()<<" "<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ] + fitMup4vect[ muIdxp22 ]  ).M()<<" "<<runNum<<" "<<evtNum<<" "<<lumiNum<<" "<<NumDup<<" "<< (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp22 ]).M()<<" "<< (fitMup4vect[ muIdxp12 ]+ fitMup4vect[ muIdxp21 ]).M()<<endl;			
...
// myoutfileevtmix<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).Px()<<" "<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).Py()<<" "<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).Pz()<<" "<<(fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Px()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Py()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Pz()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M()<<" "<<fitMup4vect[ muIdxp11 ].Px()<<" "<<fitMup4vect[ muIdxp11 ].Py()<<" "<<fitMup4vect[ muIdxp11 ].Pz()<<" "<<fitMup4vect[ muIdxp12 ].Px()<<" "<<fitMup4vect[ muIdxp12 ].Py()<<" "<<fitMup4vect[ muIdxp12 ].Pz()<<" "<<fitMup4vect[ muIdxp21 ].Px()<<" "<<fitMup4vect[ muIdxp21 ].Py()<<" "<<fitMup4vect[ muIdxp21 ].Pz()<<" "<<fitMup4vect[ muIdxp22 ].Px()<<" "<<fitMup4vect[ muIdxp22 ].Py()<<" "<<fitMup4vect[ muIdxp22 ].Pz()<<" "<<(*MyFourMuonVtxCL)[myFourMuIdx]<<" "<<myfourmuonmass<<" "<<(*MyFourMuonMassErr)[myFourMuIdx]<<" "<<runNum<<" "<<evtNum<<" "<<lumiNum<<endl;
...
//myoutfilesignal2<<myfourmuonmass<<" "<<(*MyFourMuonMassErr)[myFourMuIdx]<<endl;
							myoutfilesignal2<<myfourmuonmass<<" "<<(*MyFourMuonMassErr)[myFourMuIdx]<<" "<<myMupMumMass[mypidx][0]<<" "<<myMupMumMassErr[mypidx][0]<<" "<< (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M()<<" "<<(fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M()<<" "<<(fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M() << " "<< (fitMup4vect[ muanotherIdxp21 ]+ fitMup4vect[ muanotherIdxp22 ]).M()<<endl;			
...
//dr means deltaeta
			...
			//myV1VsMuMuMassAfterVVmassanotherpair->Fill((fitMup4vect[ muanotherIdxp11 ]+ fitMup4vect[ muanotherIdxp12 ]).M(), (fitMup4vect[ muanotherIdxp21 ]+ fitMup4vect[ muanotherIdxp22 ]).M() ); 
				...
				//calculate minDeltaR for mu+mu- and mu+mu+

				//float myoppsitemuminDeltaR=min( rawMup4vect[ muIdxp11 ].DeltaR( rawMup4vect[ muIdxp12 ] ),
				//			  rawMup4vect[ muIdxp21 ].DeltaR( rawMup4vect[ muIdxp22 ] ));
				//float mysamesiganmuminDeltaR=min( rawMup4vect[ muIdxp11 ].DeltaR( rawMup4vect[ muIdxp12 ] ),
				//			  rawMup4vect[ muIdxp21 ].DeltaR( rawMup4vect[ muIdxp22 ] ));
				...
				//myDeltaEtavsDeltaPhi4minOppositeMuat18dot5GeV->Fill( fabs( rawMup4vect[ muIdxp11 ].Eta()- rawMup4vect[ muIdxp12 ].Eta() ),  fabs(rawMup4vect[ muIdxp11 ].DeltaPhi( rawMup4vect[ muIdxp12 ])) );
				// myDeltaEtavsDeltaPhi4minOppositeMuat18dot5GeV->Fill( fabs( rawMup4vect[ muIdxp21 ].Eta()- rawMup4vect[ muIdxp22 ].Eta() ),  fabs(rawMup4vect[ muIdxp21 ].DeltaPhi( rawMup4vect[ muIdxp22 ])) );
				//myDeltaEtavsDeltaPhi4minOppositeMuat18dot5GeV->Fill( fabs( rawMup4vect[ muanotherIdxp11 ].Eta()- rawMup4vect[ muanotherIdxp12 ].Eta() ),  fabs(rawMup4vect[ muanotherIdxp11 ].DeltaPhi( rawMup4vect[ muanotherIdxp12 ])) );
				//myDeltaEtavsDeltaPhi4minOppositeMuat18dot5GeV->Fill( fabs( rawMup4vect[ muanotherIdxp21 ].Eta()- rawMup4vect[ muanotherIdxp22 ].Eta() ),  fabs(rawMup4vect[ muanotherIdxp21 ].DeltaPhi( rawMup4vect[ muanotherIdxp22 ])) );

				//myDeltaEtavsDeltaPhi4minSameSignMuat18dot5GeV->Fill( fabs( rawMup4vect[ muIdxp11 ].Eta()- rawMup4vect[ muIdxp21 ].Eta() ),  fabs(rawMup4vect[ muIdxp11 ].DeltaPhi( rawMup4vect[ muIdxp21 ])) );
				////myDeltaEtavsDeltaPhi4minSameSignMuat18dot5GeV->Fill( fabs( rawMup4vect[ muIdxp12 ].Eta()- rawMup4vect[ muIdxp22 ].Eta() ),  fabs(rawMup4vect[ muIdxp12 ].DeltaPhi( rawMup4vect[ muIdxp22 ])) );
				...
				//myDalitzmY1smupmasssqVsmmumumasssqat18GeV->Fill(  (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ] + fitMup4vect[ muIdxp21 ]  ).M2(), (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M2()  );
							...
							myUpsilonEtaVsPtat18dot5GeV->Fill( (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).Eta(), (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).Pt() );
							myMuMuEtaVsPtat18dot5GeV->Fill( (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Eta(), (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Pt() );
							myFourMuonEtaVsPtat18dot5GeV->Fill(  (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ] + fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Eta(),  (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ] + fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Pt());
							myFourMuonMomentumat18dot5GeV->Fill(  (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ] + fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).P());
			...
			myUpsilonEtaVsPtatSideband->Fill( (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).Eta(), (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).Pt() );
			myMuMuEtaVsPtatSideband->Fill( (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Eta(), (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Pt() );
			myFourMuonEtaVsPtatSideband->Fill(  (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ] + fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Eta(),  (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ] + fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).Pt());

			myFourMuonMomentumatSideband->Fill(  (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ] + fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).P());
	...
	/*
myMuMu1MassWithlightmesonatunselectedcombination->Write();
myMuMu2MassWithlightmesonatunselectedcombination->Write();
myMuMu1MassWithoutlightmesonatunselectedcombination->Write();
myMuMu2MassWithoutlightmesonatunselectedcombination->Write();
