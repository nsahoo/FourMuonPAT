...
void mysimultaneousfit()
{
  ...
  //4mu without light mesons in unselcted pair
...
//        mean.setVal( 1.84714e+01 ); 
  ...
  RooDataSet *data_4munolm = model4munolm_sig.generate(RooArgSet(x),2820*10) ;
  RooDataSet *data_2mu2ele = model2mu2ele_sig.generate(RooArgSet(x),520*10);
   */
  
  RooArgSet variables_4munolm;
  variables_4munolm.add(x);
  //data_4munolm= &(RooDataSet::read("haha.txt",variables_4munolm,"Q"));
...
SigFitNLL=myfitresult->minNll();
  ...
  RooPlot* frame1 = x.frame(Bins(65),Title("Upsilon mu+mu- w/o ligh meson")) ;
...
//  frame2->GetYaxis()->SetTitle("pull");
  ...
  // Make a frame for the physics sample 4m with light meson in unselected pair
  RooPlot* frame3 = x.frame(Bins(65),Title("Upsilon mu+mu- w/ ligh meson")) ;  
...
//  frame->GetYaxis()->SetTitle("pull");
  ...
  // Plot all data_4muwithlm tagged as physics sample
