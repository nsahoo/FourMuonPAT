...
void TwoMuTwoEleBkgFit() 
{
  cout<<"#################################"<<endl;
  cout.precision(30);
  TFile *f1 = TFile::Open("2mu2elemass.root");
  ...
  RooRealVar varMyXmass("varMyXmass", "m(#mu^{+}#mu^{-}e^{+}e^{-}) [GeV]",JpsiMassmin,JpsiMassmax);
...
*/
 ...
 //  RooAbsPdf * BkgPolPdf = new RooGenericPdf("BkgPolPdf","BkgPolPdf","pow( (m0-varMyXmass),p1 )/pow(varMyXmass,p2+p3*TMath::Log(varMyXmass))",RooArgSet(varMyXmass,m0,p1,p2,p3));
  ...
  // myEventMixingMassHistfinebin->Rebin();
  RooDataHist alldata("alldata","alldata",varMyXmass, my2Mu2EleMassHist);
