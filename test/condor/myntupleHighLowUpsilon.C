...
{  
  ...
  //ofstream myoutfileevtmix("myfourmuonevtmix.txt");
	...
	TString myhigheroutname=outputname+"HigherisUpsilom.txt";
	ofstream myhigherout(myhigheroutname);

	TString myloweroutname=outputname+"LowerisUpsilon.txt";
	ofstream mylowerout(myloweroutname);
	// ofstream myoutfiledimu("mydimuon.txt");




	TH1F* myMuMuonlymass = new TH1F("myMuMuonlymass","myMuMuonlymass",1000,0,500.0);  
	myMuMuonlymass->GetXaxis()->SetTitle("m(#mu^{+}#mu^{-}) GeV");


	TH1F* myMuMuMuMumassV1mumubelowV1 = new TH1F("myMuMuMuMumassV1mumubelowV1","myMuMuMuMumassV1mumubelowV1",1000,0,100.0);
	myMuMuMuMumassV1mumubelowV1->GetXaxis()->SetTitle("m(#mu^{+}#mu^{-}#mu^{+}#mu^{-}) GeV");

	TH1F* myHigherUps = new TH1F("myHigherUps","myHigherUps",2000,0,20.0);
	TH1F* myLowermumu = new TH1F("myLowermumu","myLowermumu",2000,0,20.0);

	TH1F* myLowerUps = new TH1F("myLowerUps","myLowerUps",2000,0,20.0);
	TH1F* myHighermumu = new TH1F("myHighermumu","myHighermumu",2000,0,20.0);

	TH2F* myUpsvsMuMu = new TH2F("myUpsvsMuMu","myUpsvsMuMu",2000,0,20.0,2000,0,20.0);
...
/*
			...
			for(int i=1; i<3; i++)
			{
				if( (fitMup4vect[ myCombIdx[i].p11 ]+ fitMup4vect[ myCombIdx[i].p12 ]).M()<(fitMup4vect[ myCombIdx[i].p21 ]+ fitMup4vect[ myCombIdx[i].p22 ]).M()  ) swap_one_paired_pair( myCombIdx[i] ); 
					//for have first pair to have bigger mass
			}	*/
	...
	// (N.B. "All" condition not saved => muQual  must have 1 for GlobalMu (1), TrackerMu(3). TMArbitrated(4), TMLastStationLoose (7)
	    ...
	    //we have two pairs, but only fill one time if at least one pair satisfy it
				...
				for(int mypidx=0; mypidx<3; mypidx++)
	    // loop over the two combinations with zero ch
				{
		...
		//for this selected pair
		     ...
		     // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~change end suleyman~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
							// &&  (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M()	> 8.5
							// &&  (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M() < 11.0
							// &&  (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M()	> 4.0
							// &&  (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M() < 11.0	
			 				// && myNumGoodTightMuon>=2 
							)
						{
							float myUpsMass = (fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M();
							float myMuMuMass = (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M();
							// if(NumDup<1) 
							{			
								myMuMuMuMumassV1mumubelowV1->Fill( myfourmuonmass );

								myUpsvsMuMu->Fill((fitMup4vect[ muIdxp11 ]+ fitMup4vect[ muIdxp12 ]).M(), (fitMup4vect[ muIdxp21 ]+ fitMup4vect[ muIdxp22 ]).M());

								
								if(myUpsMass > myMuMuMass) 
								{
									myHigherUps->Fill(myUpsMass);
									myLowermumu->Fill(myMuMuMass);
									myhigherout<< myUpsMass<< " " << myMuMuMass << " " << myrap1 << " "<< myrap2 
								...
								{
									myLowerUps->Fill(myUpsMass);
									myHighermumu->Fill(myMuMuMass);
									mylowerout<< myUpsMass<< " " << myMuMuMass << " " << myrap1 << " "<< myrap2 
									<<" "<< rawMup4vect[ muIdxp11 ].Pt()<< " " <<  rawMup4vect[ muIdxp12 ].Pt()
									<<" "<< rawMup4vect[ muIdxp21 ].Pt()<< " " <<  rawMup4vect[ muIdxp22 ].Pt()
									<<" "<<endl;
								}
