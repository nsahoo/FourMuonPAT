...
TFile *f1 = TFile::Open("myMinBiasDimuon.root");
	...
	//hh->Rebin(5);
	double JpsiMassmin=65., JpsiMassmax=115.0;

	RooRealVar x("x", "Mass (GeV/c^{2})", JpsiMassmin, JpsiMassmax);
	RooRealVar mean("Mean", "Mean" ,91,90,93) ;
	


	RooRealVar sigma_bw0("sigma_bw0", "sigma_bw0" , 3.72192e-01 , 0.05,5.2) ; //0.2, 0.05,0.5
	RooRealVar sigma_cb0("sigma_cb0", "sigma_cb0" , 0.4, 0.01, 5.5) ;
	...
	RooRealVar n("n","", -10.0, 100.0); //0.0, 5.0
	RooRealVar alpha("alpha","", -5.0, 5.5);
	...
	RooRealVar MyXfrac("MyXfrac","frac ",0.0302285,0.0,1.0) ;
...
*/
	...
	RooRealVar c1("c1", "c1", -0.5 , -2.0, 2.0);
	RooRealVar c2("c2", "c2", .0, -2.3, 2.3);
        RooRealVar c3("c3", "c3", .0, -2.3, 2.3);

	RooAbsPdf *  BkgPdf = new RooChebychev("BkgPdf","BkgPdf",x,RooArgSet(c1,c2,c3));
