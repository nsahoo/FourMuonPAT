...

{
	...
	char buff[60];//array for symbols input
	size_t num;


	float last_max_Xvalue = 0;
	float max_Xvalue;
	float last_min_Xvalue = 40.;
	float min_Xvalue;

	float last_max_Yvalue = 0.;
        float max_Yvalue;
        float last_min_Yvalue = 1.;
        float min_Yvalue;
...
//	char * filename = "myfile02.txt";
	...
	while ((num = fread(buff,1,60,fp)) > 0) //reading 512-symbols block. num is the number of read symbols. 
	{
		for (i=0;i<num;++i)
		{
			if (buff[i] == '\n')
